...
class ShiftTableWidget(QWidget):
    def __init__(self, shiftModel, rowHeaderModel, columnHeaderModel, countModel):
        ...
        self.rowHeaderView.minimumSizeHint()
        ...
        self.baseLayout.setColumnStretch(1,2)
...
# ---------------------------------------------------------------
    ...
    def convertViewSizeHint(self):
        colwidth = 0
        for col in range(self.shiftModel.columnCount()):
            colwidth += self.shiftView.columnWidth(col)
        self.shiftView.setMaximumWidth(colwidth)
        self.columnHeaderView.setMaximumWidth(colwidth)
        self.scrollView.setMaximumWidth(colwidth)
        self.shiftView.setSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Fixed)
        self.columnHeaderView.setSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Fixed)
        self.scrollView.setSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Fixed)
        
    def setWidgetMaximumSize(self):
        colwidth = 0
        for col in range(self.rowHeaderModel.columnCount()):
            colwidth += self.rowHeaderView.columnWidth(col)

        for col in range(self.shiftModel.columnCount()):
            colwidth += self.shiftView.columnWidth(col)

        for col in range(self.countModel.columnCount()):
            colwidth += self.countView.columnWidth(col)

        colwidth += self.countView.verticalScrollBar().width() +2
        # print(colwidth)
        # colwidth -= self.shiftView.verticalScrollBar().width()
        # colwidth -= self.rowHeaderView.verticalScrollBar().width()
        # print(colwidth)
