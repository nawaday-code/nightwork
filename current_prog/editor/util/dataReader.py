...
class DatNames(Enum):
    ...
    skill = 'skill.dat'
    tokai_calendar = 'tokai-calendar.csv'
...
class DataReader(Members):
    ...
    def readConfigvar(self, datPath: str = ''):
        ...
        next_month = 1 if self.date.month == 12 else self.date.month + 1
        previous_month = 12 if self.date.month == 1 else self.date.month -1
        previous_year = self.date.year -1 if self.date.month == 1 else self.date.year


        self.previous_month = [datetuple for datetuple in cal.itermonthdays3(
            previous_year, previous_month) if datetuple[1] == previous_month][-int(data['iota'][0]):]
        ...
        self.next_month = [datetuple for datetuple in cal.itermonthdays3(
            next_year, next_month) if datetuple[1] == next_month]
    ...
    def readTokaiCalendar(self, datPath: str = ''):
        try:
            inputData = open(datPath, 'r', encoding='cp932')
        except FileNotFoundError as ex:
            path = os.path.join(self.rootPath, DatNames.tokai_calendar.value)
            inputData = open(path, 'r', encoding='cp932')
        ...
        for row in inputData:
            if not row[0] == '#':
                ...
                key = datetime.datetime.strftime(key, '%Y-%m-%d')
                data.setdefault(key, value)
        self.tokai_calendar = data
        inputData.close()

    
# 病院の休診日が固定されないため、休診日を記述したcsvファイルで管理することにする
# class JapanHoliday():
#     """
