...
class DataModel():
    def __init__(self):
        ...
        shiftdata = self.readshift(os.path.join(ROOT_DIR, 'shift.dat'), Tdict)
        previousdata = self.readshift(os.path.join( ROOT_DIR, 'previous.dat'), Tdict)
        requestdata = self.readshift(os.path.join(ROOT_DIR, 'request.dat'), Tdict)
    ...
    def getuid(self):
        path = os.path.join(ROOT_DIR, 'shift.dat')
    ...
    def readstaffinfo(self):
        path = os.path.join(ROOT_DIR, 'staffinfo.dat')
        result = pd.read_csv(path, header=None, names=['uid', 'id', 'staffname'])
        
        return result

    def readshift(self, path, Tdict):
        # path = os.path.join(config.DATA_DIR, ROOT_DIR, 'shift.dat')
        shift = pd.read_csv(path, header=None, skiprows=1, names=['uid', 'date', 'shift'])
    ...
    def readrequest(self):
        shiftDict = self.readshiftDict()
        path = os.path.join(ROOT_DIR, 'request.dat')
    ...
    def readprevious(self):
        shiftDict = self.readshiftDict()
        path = os.path.join(ROOT_DIR, 'previous.dat')
    ...
    def readconfigvar(self):
        path = os.path.join(ROOT_DIR, 'configvar.dat')
        data = []
    ...
    def readshiftDict(self):
        shiftDict = {}
        path = os.path.join(ROOT_DIR, 'converttable.dat')
...
class JapanHoliday:
    ...
    def __init__(self, encoding='cp932'):
        _path = os.path.join(ROOT_DIR,'syukujitsu.csv')
