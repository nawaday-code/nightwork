...
def calc_schedule():
    ...
    # 勤務間隔変数
    i1 = pulp.LpVariable.dicts('i1', [(n, t) for n in Nr for t in Tr], cat='Binary')
    i2 = pulp.LpVariable.dicts('i2', [(n, t) for n in Nr for t in Tr], cat='Binary')
    i3 = pulp.LpVariable.dicts('i3', [(n, t) for n in Nr for t in Tr], cat='Binary')
    i4 = pulp.LpVariable.dicts('i4', [(n, t) for n in Nr for t in Tr], cat='Binary')
    # i5 = pulp.LpVariable.dicts('i5', [(n, t) for n in Nr for t in Tr], cat='Binary')
    # 各技師の合計夜勤・休日日勤回数
    s = pulp.LpVariable.dicts('s', [(n) for n in Nr], cat='Integer')
    # 各技師の合計夜勤・休日日勤回数の平均偏差
    d = pulp.LpVariable.dicts('d', [(n) for n in Nr], lowBound=0, cat='Continuous')
    # 連休変数''
    h2 = pulp.LpVariable.dicts('h2', [(n, t) for n in Nr for t in Tr], cat='Binary')
    h3 = pulp.LpVariable.dicts('h3', [(n, t) for n in Nr for t in Tr], cat='Binary')
    h4 = pulp.LpVariable.dicts('h4', [(n, t) for n in Nr for t in Tr], cat='Binary')

    # 目的関数（soft constraints）・・・
    model += pulp.lpSum([x[n, t, w] for n in Ndum for t in Tr for w in W1]) \
        + lam[0] * pulp.lpSum([i1[n, t] for n in Nr for t in Tr]) \
        + lam[1] * pulp.lpSum([i2[n, t] for n in Nr for t in Tr]) \
        + lam[2] * pulp.lpSum([i3[n, t] for n in Nr for t in Tr]) \
        + lam[3] * pulp.lpSum([i4[n, t] for n in Nr for t in Tr]) \
        + lam[4] * pulp.lpSum([d[n] for n in Nr])
    ...
    for n in Nr: 
        ...
        # model += pulp.lpSum([x[n, t, w] for t in Tr for w in W[:7]]) <= epsilon
